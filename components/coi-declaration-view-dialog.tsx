...
interface CoiDeclarationViewDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function CoiDeclarationViewDialog({
  open,
  onOpenChange,
}: CoiDeclarationViewDialogProps) {
  const d = DUMMY_DECLARATION

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex flex-col p-0 gap-0 h-[90vh] overflow-hidden sm:max-w-3xl w-full">
